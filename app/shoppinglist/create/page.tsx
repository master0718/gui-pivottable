...
import { ShoppingListSearchService } from "@/service/shopping-list/shopping-list-search-service";
import { getShoppingListSearchForm } from "@/redux/features/shopping-list-search-form/reducer";

import { ShoppingTableTabView } from "@/components/page-components/shopping-list/shopping-table/shopping-table-tab-view";
import { ShoppingTableListView } from "@/components/page-components/shopping-list/shopping-table/shopping-table-list-view";
import { ShoppingTableTreeView } from "@/components/page-components/shopping-list/shopping-table/shopping-table-tree-view";
import { ShoppingListPopupTable } from "@/components/page-components/shopping-list/shopping-list-popup-table";
...
const CreateShoppingList = () => {
	...
	} = useActions("shoppingListSearchForm");

	const shoppingListSearchForm = useTypedSelector(getShoppingListSearchForm);

	const shoppingListSearchService =
		ShoppingListSearchService.getInstance("token");
	// call api which get all meso cycles
	const loadMesoCycleItems = () => {
		const mesoCycleItems = shoppingListSearchService.getMesoCycleItems();
		setMesoCycleItems(mesoCycleItems);
	};

	// call api which get all micro cycles
	const loadMicroCycleItems = () => {
		const microCycleItems = shoppingListSearchService.getMicroCycleItems();
		setMicroCycleItems(microCycleItems);
	};

	// call api which get all day types
	const loadDayTypeItems = () => {
		const dayTypeItems = shoppingListSearchService.getDayTypeItems();
		setDayTypeItems(dayTypeItems);
	};

	// call api which get all days
	const loadDayItems = () => {
		const dayItems = shoppingListSearchService.getDayItems();
		setDayItems(dayItems);
	};

	// call api which get all activity types
	const loadActivityTypeItems = () => {
		const activityTypeItems = shoppingListSearchService.getActivityTypeItems();
		setActivityTypeItems(activityTypeItems);
	};

	// call api which get all activity names
	const loadActivityNameItems = () => {
		const activityNameItems = shoppingListSearchService.getActivityNameItems();
	...
	const loadActivityStartTimeItems = () => {
		const activityStartTimeItems =
			shoppingListSearchService.getActivityStartTimeItems();
	...
	const loadActivityEndTimeItems = () => {
		const activityEndTimeItems =
			shoppingListSearchService.getActivityEndTimeItems();
	...
	const loadMealTargetMacroItems = () => {
		const mealTargetMacroItems =
			shoppingListSearchService.getMealTargetMacroItems();
				...
				<Card className="lg:block flex-none w-80">
					...
					<CardContent className="flex flex-col hidden lg:block p-2">
						<div className="flex flex-col gap-2">
							<SelectFilter
								props={shoppingListSearchForm.mesoCycleField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.microCycleField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.dayTypeField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.dayField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.activityTypeField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.activityNameField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.activityStartTimeField}
								module={"shoppingListSearchForm"}
							/>
							<SelectFilter
								props={shoppingListSearchForm.activityNameField}
							...
							/>
							<SelectFilter
								props={shoppingListSearchForm.mealTargetMacroField}
								module={"shoppingListSearchForm"}
							/>
						</div>
					</CardContent>
								...
								<div className="flex justify-end gap-2">
									...
									<Dialog>
										<DialogTrigger asChild>
											<Button className="h-8">View shopping list</Button>
										</DialogTrigger>
										<DialogContent className="max-w-[80%]">
											...
											<div className="flex items-center justify-end gap-1">
												<Button className="h-8">Print selected</Button>
												<Button className="h-8">Export selected</Button>
												<Button className="h-8">Send selected</Button>
											</div>
											<ShoppingListPopupTable />
										</DialogContent>
									</Dialog>
								</div>
							</div>
						...
						<CardContent>
							<Tabs defaultValue="tab-view" className="">
								...
								<TabsContent value="tab-view">
									<ShoppingTableTabView />
								</TabsContent>
								<TabsContent value="list-view">
									<ShoppingTableListView />
								</TabsContent>
								<TabsContent value="tree-view">
									<ShoppingTableTreeView viewMode="scroll" />
								</TabsContent>
							</Tabs>
							<div className="flex justify-between items-center">
