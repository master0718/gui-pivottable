...
import { SupplementListSearchForm, SearchItem } from "./types"

const initialState: SupplementListSearchForm = {
	mesoCycleField: {
		key: "MESO_CYCLE",
		title: "MesoCycles",
	...
	mealTargetMacroField: {
		key: "MEAL_TARGET_MACRO",
		title: "Meal target macros",
	...
	mealTargetMacroItems: [],
}

export const supplementListSearchFormSlice = createSlice({
	name: "supplementListSearchForm",
	...
	reducers: {
		...
		updateSearchItem: (state, action: PayloadAction<SearchItem>) => {
			...
			switch (searchItem.key) {
				...
				case "ACTIVITY_ENDTIME":
					...
					break;
				case "MEAL_TARGET_MACRO":
					state.mealTargetMacroField = {
						...state.mealTargetMacroField,
						items: state.mealTargetMacroField.items.map((item) => {
							if (item.id === searchItem.id) {
								return { ...item, status: searchItem.status };
							}
							return item;
		...
		filterSearchItem: (state, action: PayloadAction<{ key: string, searchText: string }>) => {
			...
			switch (key) {
				...
				case "MEAL_TARGET_MACRO":
					const mealTargetMacroItems = state.mealTargetMacroItems.map((mealTargetMacroItem) => {
						const matchingItem = state.mealTargetMacroField.items.find((item) => item.id === mealTargetMacroItem.id);
						if (matchingItem) {
							mealTargetMacroItem.status = matchingItem.status;
						}
						return mealTargetMacroItem;
					});
					const filteredMealTargetMacroItems = mealTargetMacroItems.filter((mealTargetMacroItem) =>
						mealTargetMacroItem.label.includes(searchText)
					);
					state.mealTargetMacroField = {
						...state.mealTargetMacroField,
						items: filteredMealTargetMacroItems
					}
					break;
				default:
					break;
		...
		// get all Activity end times and set the initial state
		setMealTargetMacroItems: (state, action: PayloadAction<Array<SearchItem>>) => {
			state.mealTargetMacroItems = action.payload;
			state.mealTargetMacroField = {
				...state.mealTargetMacroField,
				items: action.payload
			}
...
// selectors
export const getSupplementListSearchForm = (state: RootState) => state.supplementListSearchForm

export const { reducer } = supplementListSearchFormSlice;